    ...
    if (atual != NULL) {
        ...
        if (impar == 0) {
            ...
            if (atual->elemento.cpf % 2 == 0) {
                ...
                cout << "Nome: " << atual->elemento.chave << " | " << "Cpf: " << atual->elemento.cpf << " | " << "Cod: " << atual->elemento.chave << endl;
...
void avl::imprimeAutorizados(int dia) {
    imprimeAutorizadosAux(this->raiz, dia % 2 == 1);
}

// utiliza o nó atual e seu nível na árvore (para facilitar visualização)
bool avl::verificaAutorizado(tipoChave chave, int dia) {
    noh* variavel = buscaAux(chave);
    ...
    if (variavel == NULL) {
        ...
        if (((chave%2 == 1) and (dia%2 == 1)) or ((chave%2 == 0) and (dia%2 == 0))) {
            return true;
        }
        else {
            return false;
